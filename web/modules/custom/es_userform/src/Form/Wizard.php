...
namespace Drupal\es_userform\Form;
...
class Wizard extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $no_js_use = FALSE) {
    $url = Url::fromUri('internal:/ici/es-userform/wizard-nojs');
    $link = Link::fromTextAndUrl($this->t('ici/es-userform/wizard-nojs'), $url)
      ->toString();

    // Prepare link for multiple arguments.
    $urltwo = Url::fromUri('internal:/ici/es-userform/wizard');
    $linktwo = Link::fromTextAndUrl($this->t('ici/es-userform/wizard'), $urltwo)
      ->toString();
    ...
    if ($form['step']['#value'] > 1) {
      $form['actions']['prev'] = [
        ...
        '#submit' => ['::prevSubmit'],
        '#ajax' => [
          'wrapper' => 'es-userform-wizard-wrapper',
          'callback' => '::prompt',
        ],
      ];
    }
    ...
    $form['#prefix'] = '<div id="es-userform-wizard-wrapper">';
    $form['#suffix'] = '</div>';
