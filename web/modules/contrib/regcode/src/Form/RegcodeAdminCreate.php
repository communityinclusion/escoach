  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    for ($i = 0; $i < (int) $form_state->getValue(['regcode_create_number']); $i++) {
      ...
      if (regcode_save($code, REGCODE_MODE_SKIP)) {
        $this->messenger()->addStatus(t('Created registration code (%code)', [
          '%code' => $code->code,
        ]));
      }
      else {
        $this->messenger()->addWarning(t('Unable to create code (%code) as code already exists', [
          '%code' => $code->code,
        ]));
      }
    }
